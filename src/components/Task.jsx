...
export default function Task({ task, onClick, columnId, isOverlay, index, totalTasks, onDelete, onEdit, isDoneColumn }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  {...listeners}
  className={`
    relative ${isDoneColumn ? 'bg-green-100 dark:bg-green-900/20' : 'bg-gray-100 dark:bg-gray-700'} p-3 rounded-md shadow-sm
  ...
  role="button"
  aria-label={`Task ${task.title}, ${completedSubtasks} trên ${totalSubtasks} subtasks hoàn thành${task.deadline ? `, thời hạn ${formatDeadline(task.deadline)}` : ''}${isCompleted ? ', đã hoàn thành' : ''}${task.priority ? `, ưu tiên ${task.priority.toLowerCase()}` : ''}`}
>
