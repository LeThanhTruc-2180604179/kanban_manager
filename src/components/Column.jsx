...
export default function Column({ column, tasks, openTaskModal, setEditingTask, updateColumn, columns, isOverlay, onDeleteTask, onDeleteColumn }) {
    ...
    const calculateMaxHeight = () => {
      ...
      if (container) {
        ...
        if (columnElements.length > 0) {
          const heights = Array.from(columnElements).map(col => col.scrollHeight);
  ...
  const handleSaveEditColumn = () => {
    ...
    if (columns.some(col => col.id !== column.id && col.name.toLowerCase() === newColumnName.trim().toLowerCase())) {
      setErrorMessage('Tên cột đã tồn tại');
      return;
    }
        ...
        <div ref={formRef} className="mb-4">
          ...
          <div className="flex justify-end space-x-2 mt-2">
            ...
            <button
              ...
              className="p-1 bg-white border border-gray-300 rounded hover:bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:hover:bg-gray-600"
              aria-label="Hủy chỉnh sửa tên cột"
            >
              <svg className="w-5 h-5 text-gray-600 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
